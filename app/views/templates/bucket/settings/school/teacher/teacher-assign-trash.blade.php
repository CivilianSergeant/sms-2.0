            ...
            <section class="panel">
                <header class="panel-heading">
                    ...
                    <span class="tools pull-right col-lg-3">
                        ...
                        {{ Form::open(array('url'=>'school/teacher-assign','class'=>'col-lg-4','method'=>'get')) }}
                                <button class="btn btn-primary panel-btn" type="submit"><i class="fa fa-list"></i> List</button>
                        {{ Form::close() }}
                        {{ Form::open(array('url'=>'school/clear-trash-teacher-assign','class'=>'col-lg-2', 'id'=>'clearTrashTeacherFrm', 'method'=>'get')) }}
                    ...
                    <table class="display table table-bordered table-striped" id="dynamic-table">
                        ...
                        @if(count($teachers))
                            @foreach($teachers as $index=> $teacher)
                                <tr>
                                    <td>{{{($index + 1)}}}</td>
                                    <td>{{{ucfirst($teacher->getTeacher->name) or ''}}}</td>
